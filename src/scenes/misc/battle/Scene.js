...
export default class extends Phaser.Scene {
  ...
  init(data) {
    ...
    if (Object.keys(this.config.player).length === 0) {
      // console.error('Player doesnt exist...exiting.');
      return;
    }
    if (Object.keys(this.config.enemy).length === 0) {
  ...
  nextTurn() {
    ...
    let endBattle = this.checkEndBattle();
    console.log('checkEndBattle: ' + (endBattle ? 'true' : 'false'));
    if (endBattle) {
      if (!this.config.player.team.hasLivingPokemon()) {
        this.logger.addItem(this.config.player.name + ' has no usable Pokemon remaining.');
      }
      if (!this.config.enemy.team.hasLivingPokemon()) {
        this.logger.addItem(this.config.enemy.name + ' has no usable Pokemon remaining.');
      }
    ...
    if (Object.values(this.actions).filter(n => n).length != 2) {
      ...
      if (this.playerTurn === 'player') {
        ...
        //this.logger.addItem(this.activeMon[this.playerTurn].getName() + ' Turn..');
      ...
      order.forEach(player => {
        ...
        switch (this.actions[player].type) {
          case 'attack':
            info = this.activeMon[player].attack(this.actions[player].target, this.actions[player].move);
            break;
          case 'npc_attack':
            info = this.activeMon[player].attackRandomMove(this.actions[player].target);
            break;
        }
        if (Object.keys(info).length > 0) {
          ...
          logMsg += ' for ' + info.damage + '('+this.actions[player].target.currentHp+') damage';
          this.logger.addItem(logMsg);
          if (info.critical === 2) {
            this.logger.addItem('It was a critical hit!');
          }
          console.log('effective', info);
  ...
  receivePlayerSelection(action, index) {
    if (action == 'attack') {
      ...
      console.log('player action set!', this.actions.player, move);
      console.groupEnd();
    }
    this.time.addEvent({ delay: 1000, callback: this.nextTurn, callbackScope: this });
  }
